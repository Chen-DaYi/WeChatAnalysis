...
class Client:
    ...
    def send_message_raw(self, prompt, conversation_id, attachment=None):
    ...
    def send_message_withfilecontent(
        self, prompt, conversation_id, file_path=None, file_content=None
    ):
        url = "https://claude.ai/api/append_message"

        # 1. 处理要上传的文件，根据地址判断类型、大小、名称。
        # to-do: 内容就不读了，直接传进来
        attachment = {}
        if file_path:
        ...
        response = requests.post(url, headers=headers, data=payload, stream=True)
        self.decoded_data = response.content.decode('utf-8')  # 为了方便后面排查问题，把这个数据存下来
        data = [json.loads(i.replace('\\n\\n', '\\n')[6:]) for i in self.decoded_data.strip().split('\n') if len(i) > 0]
        answer = ''.join([i['completion'] for i in data]).strip()
        return answer
